     ...
     <div class="card">
        ...
        @endif
          ...
          <div class="form-group">
            ...
            <Input type="text" class="form-control" id="exampleInputEmail1"  placeholder="title" name="title" value="{{old('title') ?? $post->title}}">
          </div>

          <div class="form-group">
            <label for="exampleFormControlFile1">画像ファイル</label>
            <input type="file" class="form-control-file" id="exampleFormControlFile1" name="gazou_path" value="{{old('gazou_path') ?? $post->gazou_path}}">
          </div>

          <div class="form-group">
            <label for="exampleFormControlSelect1">季節</label>
            <select class="form-control" id="exampleFormControlSelect1" name="scategory_id" value="{{old('scategory_id') ?? $post->scategory_id}}">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="exampleFormControlSelect2" name="tcategory_id" value="{{old('tcategory_id') ?? $post->tcategory_id}}">
              <option selected="">選択する</option>
            ...
            <div class="form-group">
              ...
              <textarea class="form-control" rows="5" id="comment" name="content" value="{{old('content') ?? $post->content}}"></textarea>
            </div>

            <input type="hidden" name="user_id" value="{{Auth::id()}}">

            <button type="submit" class="btn btn-peimary">決定</button>
            <button type="reset" class="btn btn-secondary" onclick="location.href='{{route('posts.page')}}'">キャンセル</button>
         </form>
        </div>
      </div>
    </div>
